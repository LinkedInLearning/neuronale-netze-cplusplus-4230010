...
bool Matrix::testAddition()
{
    Matrix A = { {1,0,0}, {0,1,0}, {0,0,1} };
    Matrix B = { {1,2,3}, {4,5,6}, {7,8,9} };
    Matrix C_expc = { {2,2,3}, {4,6,6}, {7,8,10} };

    Matrix C = A + B;

    Matrix boolAdd = (C == C_expc);

    const auto numCorrect = Matrix::sum(boolAdd);
    if(numCorrect == boolAdd.getDim(0)*boolAdd.getDim(1))
    {
