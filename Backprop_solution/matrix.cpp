...
std::vector<Matrix> Matrix::splitIntoBatches(const Matrix &matrix, const uint batchSize)
{
    std::vector<Matrix> res;

    const uint numRows = matrix.getDim(0);
    const uint numCols = matrix.getDim(1);

    for(uint j=0 ; j<numRows ; j+=batchSize)
    {
        Matrix matBatch;

        // Je nachdem ob wir noch genug Zeilen übrig haben wird
        // die entsprechende Matrixgröße festgelegt
        const uint curNumRows = std::min(numRows-j, batchSize);

        matBatch.setDim(curNumRows, numCols);

        for(uint k=0 ; k<curNumRows ; k++)
        {
            for(uint i=0 ; i<numCols ; i++)
            {
                matBatch.set(k, i, matrix.get(j+k, i));
            }
        }

        res.push_back(matBatch);
    }
...
void Matrix::for_each(std::function<real (const real)> &&operation)
{
    for(auto &val : *this)
    {
        val = operation(val);
...
Matrix Matrix::slice_col(const int start) const
{
    Matrix z;

    const auto numCols = this->getDim(0)-start;
    const auto numRows = this->getDim(1);
    z.setZeros(numCols, this->getDim(1));

    for(uint i=0 ; i<numCols ; i++)
    {
        for(uint j=0 ; j<numRows ; j++)
        {
            const real val = this->get(i+start, j);
            z.set(i, j, val);
        }
    }
