...
{
    auto X_prepended = X;
    X_prepended.prepend_bias();
    const Matrix weighted_sum1 = X_prepended.matmul(w1);
    auto h = sigmoid(weighted_sum1);

    auto h_with_bias = h;
    h_with_bias.prepend_bias();

    const Matrix weighted_sum2 = h_with_bias.matmul(w2);
    auto y_hat = softmax(weighted_sum2);
